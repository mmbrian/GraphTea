...
//            if (z.key.compareTo(x.key) < 0) {
    ...
    // @param: node, a RedBlackNode
    // @param: node, the node with the largest key rooted at node
    ...
    public RedBlackNode<T> treeSuccessor(RedBlackNode<T> x) {
        ...
        // if x.right is not nil, call treeMinimum(x.right) and
        // return it's value
        if (!isNil(x.right)) {
    ...
    // @param: x, a RedBlackNode whose successor we must find
    // @return: return's the node the with the previous largest key
    // from x.key
    public RedBlackNode<T> treePredecessor(RedBlackNode<T> x) {

        // if left is not nil, return the maximum node rooted at this 
        // node's left child
        if (!isNil(x.left)) {
        ...
        // while x is it's parent's left child...
...
//        RedBlackNode<T> z = search(v.key);
    ...
    private void removeFixup(RedBlackNode<T> x) {
        ...
        while (x != getRoot() && x.color == RedBlackNode.BLACK) {
    ...
    public RedBlackNode<T> search(T key) {
        ...
        RedBlackNode<T> current = getRoot();
    ...
    public int numGreater(T key) {
        ...
        return findNumGreater(getRoot(), key);
    ...
    public int numSmaller(T key) {
        ...
        return findNumSmaller(getRoot(), key);
    ...
    public List<T> getGreaterThan(T key, Integer maxReturned) {
        ...
        getGreaterThan(getRoot(), key, list);
    ...
    public int size() {
        ...
        return getRoot().numLeft + getRoot().numRight + 1;
    }// end size()

    /**
     * swaps two nodes in RedBlackTree
     * @param first 
     * @param second 
     */
    public void swapNodes(RedBlackNode<T> first, RedBlackNode<T> second) {
//        int f_numLeft = first.numLeft,
//                f_numRight = first.numRight;
//        RedBlackNode<T> f_parent = first.parent,
//                f_left = first.left,
//                f_right = first.right;
        T key = first.key;
...
//        first.numLeft = second.numLeft;
//        first.numRight = second.numRight;
//        
//        second.left = f_left;
//        second.right = f_right;
//        second.parent = f_parent;
//        second.numLeft = f_numLeft;
//        second.numRight = f_numRight;
    }
    
    /**
     * @return set of tree nodes ascendingly sorted
     */
    public ArrayList<RedBlackNode<T>> inOrder() {
        ArrayList<RedBlackNode<T>> ret = new ArrayList<RedBlackNode<T>>();
